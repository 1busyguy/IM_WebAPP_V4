...
export function EditActivationModal({ isOpen, onClose, activationId, userId }: EditActivationModalProps) {
  const { activations, updateActivation } = useActivationStore();
      ...
      if (activation) {
        setFormData({
          ...
          external_link: {
            ...
            image_url: activation.external_link?.image_url || ''
          }
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateActivation(activationId, updateData);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg w-[1024px] max-w-full">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Edit Activation</h2>
          <button
            onClick={onClose}
